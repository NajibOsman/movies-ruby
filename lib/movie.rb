...
class Movie
  ...
  define_method(:==) do |another_movie|
    name.==(another_movie.name).&(id.==(another_movie.id))
  end
  ...
  define_method(:update) do |attributes|
    ...
    attributes.fetch(:actor_id, []).each do |actor_id|
      DB.exec("INSERT INTO actors_movies (actor_id, movie_id) VALUES (#{actor_id}, #{id});")
    end
  end

  define_method(:actors) do
    movie_actors = []
    results = DB.exec("SELECT actor_id FROM actors_movies WHERE movie_id = #{id};")
  ...
  define_method(:delete) do
    DB.exec("DELETE FROM actors_movies WHERE movie_id = #{id};")
